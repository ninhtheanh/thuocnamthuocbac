...
function tg_facebook_comments_admin_init(){
	...
	add_settings_section(
		'tg_facebook_comments_shortcodes',
		'TG Facebook Comments Plugin - ShortCode Usage',
		'tg_facebook_comments_section_shortcodes_text',
		'tg_facebook_comments'
	);
...
// Draw the section header
function tg_facebook_comments_section_shortcodes_text() {
	echo '<p>TG Facebook Comments provides you a ShortCode <strong>[tg_facebook_comments]</strong> that can be inserted by you anywhere in any post or page or in any <b>Text</b> widget to display Facebook Comments with options and settings selected here.</p> <hr />';
...
function tg_facebook_comments_setting_input_width() {
	...
	echo '<p>Keep it blank to keep the default width of 100% of the place / DIV where Facebook comments are shown. This is desirable since it will automatically make width responsive. However, if you want, you can enter any other number (in pixels) such as 350, 500, etc., as per your theme. Please enter only numbers (and <b>no other character</b>). The mobile version of Facebook Comments will ignore such number entered by you since it uses 100% width to be responsive and fluid. Likewise, shortcode will also ignore such number and will use the 100% default width setting.<p>';
}
...
function tg_facebook_comments_setting_input_titlecomments() {
	...
	echo '<p>This is the title of the Facebook Comments block. Default value is <b>TG Facebook Comments</b>.<p>';
}
...
//shortcode for inserting Facebook Comments in post or page
function tg_facebook_comments_shortcode_default($atts) {
	...
	//set width-value 
	$width_value = $width . '="100%"' ; //it is useful to have 100% width when showing in widget through shortcode
	...
	return $contents_addl ;
	
}

add_shortcode('tg_facebook_comments', 'tg_facebook_comments_shortcode_default');
